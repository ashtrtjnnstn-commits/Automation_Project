...
### Generate billing
Go to **Billing**:
- select one student (required in UI for main workflow)
- choose start/end dates
- generate 15-day cycle advice for that student
- page shows selected student context, rendered hours, rates, deposits, old balance/credit, and resulting total due

### Record payment
Go to **Payments**:
- fill ledger fields (Date, Guardian/Client, Student, Purpose, Billing Period, Hours, Amount, Received By, Mode)
- submit payment; system keeps allocation logic and computes overpayment/balance snapshot
- use **Payments Tracker** tab for active-month records and archived-month retrieval
